...
void cBreakpoint(CBTYPE cbType, void* de)
{
	...
	{
		char delCmd[100];
		sprintf(delCmd, "bc %llX", entryPoint);
		if (DbgCmdExecDirect(delCmd))
		{
			char setCmd[100];
			sprintf(setCmd, "bph %llX", entryPoint);
			DbgCmdExecDirect(setCmd);

			_plugin_logprintf("Replaced software BP with hardware BP at: %p\n", entryPoint);
...
PLUG_EXPORT bool pluginit(PLUG_INITSTRUCT* initStruct)
{
	...
	_plugin_registercallback(initStruct->pluginHandle,
		CB_SYSTEMBREAKPOINT,
		cBreakpoint);
